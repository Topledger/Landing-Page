    ...
    return (
        <div className={styles.flexBox} ref={ref}>
              <div className={styles.info}>
                <div onClick={() => onClick(data?.attributes?.link)} style={{cursor: "pointer"}}>
                  <h4>{data?.attributes?.title}</h4>
                  <p>{data?.attributes.description}</p>
                </div>
              </div>
              <div className={styles.fav}>
                <div className={styles.shareBtn} style={{display: "flex", alignItems: "center", gap: "10px"}}>
                <p onClick={() => toggleMenu()}>
                    ...
                    <div className="ShareIcons">
                      {isComponentVisible ?  <img src="assets/images/cross-small.svg"/>  : <AiOutlineShareAlt className={styles.shareIcon} />}
                ...
                {isComponentVisible && (
                  ...
                  <div className={styles.wrapper}>
                    <div className={styles.icons}>
                      ...
                      <a href={`https://t.me/share/url?url=${data?.attributes?.link}&text=${data?.attributes?.title}`} target="_blank" rel="noreferrer">
                        {/* <BsFacebook className={styles.facebook} /> */}
                        <img src="assets/images/logos_telegram-icon.svg"/>
                      </a>
                      <a href={`https://www.facebook.com/sharer/sharer.php?u=${data?.attributes?.link}`} target="_blank" rel="noreferrer" className={styles.facebook}>
                        <MdFacebook />
                      </a>
                      </div>)}
